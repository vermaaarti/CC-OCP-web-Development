...
function appendAllQuestions(){
    for(var j=0; j<questions.length; j++){
        ...
        divBody += '<button class="btn btn-info fa fa-pencil liBtn" onclick="editRec(this)"></button>'
...
function addQuestion(){
    if(!addingRec){//check if a record is'nt already being added
        ...
        htmlDesign += '<button class="btn btn-info liBtnAdd fa fa-check" onclick="addRec(this)"></button>'
...
function editRec(id){
    if(!addingRec){
        ...
        htmlDesign += '<li style="display: flex; justify-content: center;">'
        htmlDesign += '<button class="btn btn-info liBtnAdd fa fa-check" onclick="addRec(this)"></button>'// the same addRec() function will be used
